...
_as2v_complex:
	...
	cmp esi,ebp	    ;exit because *Buffer[2] reached
	jne loop_interp_real

	;beginnig range
	fld qword [edi]     ;*Ascan[0]
	fst qword [esi]     ;*Buffer[2]

	fmul st0,st5	    ;0,2 * (*Ascan[0])
	fst st1
	fadd st0,st1
	fadd st0,st1
	fst qword [esi-16]  ;0,6 * (*Ascan[0]) ->  *Buffer[0]
	fadd st0,st1
	fstp qword [esi-8]  ;0,8 * (*Ascan[0]) ->  *Buffer[1]
   ...
   Z_loop:
	...
	jne exit_X_loop  ;even
	  ...
	  sub [esp+80],dword 8 ;*bild_compl[index_aktual]+2pixel[double]
	  jmp  X_loop

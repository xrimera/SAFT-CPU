...
voxelgenerator=unique(round(2.^(2:0.01:8)))
...
figure; imagesc(voxel,blocksize,(repmat((1:length(blocksize))',[1 length(voxel)]).*voxel.^3)./mean(timesBlocked,3)); colorbar;title('blocked')
figure; plot(voxel,(repmat((1:length(blocksize))',[1 length(voxel)]).*voxel.^3)./mean(timesBlocked,3));title('blocked')
